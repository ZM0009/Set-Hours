...
function modal(title,html,onSave){
  ...
  form.reset();
  form.onsubmit=e=>{e.preventDefault();const fd=Object.fromEntries(new FormData(form));onSave(fd);dialog.close('saved')};
  $('#modalCancel').onclick=()=>dialog.close('cancelled');
  $('#modalClose').onclick=()=>dialog.close('cancelled');
  dialog.onclick=e=>{if(e.target===dialog)dialog.close('cancelled')};
  dialog.showModal();
}
...
function openDay(date){
  const ss=data.shifts.filter(s=>isoLocal(s.start)===date);
  if(!ss.length){addManual();return}
  const s=ss[0];
  const deleteHtml='<button type="button" id="deleteShiftBtn" class="delete-shift-button">Delete Shift</button>';
  modal('Edit Shift',field('Start','start',new Date(s.start).toTimeString().slice(0,5),'time')+field('Finish','finish',new Date(s.end).toTimeString().slice(0,5),'time')+field('Travel hours','travel',s.travel,'number')+field('Production','production',s.production)+area('Notes','notes',s.notes)+deleteHtml,fd=>{
    const st=new Date(s.start),en=new Date(s.end);const [sh,sm]=fd.start.split(':');const [eh,em]=fd.finish.split(':');st.setHours(sh,sm,0,0);en.setHours(eh,em,0,0);if(en<=st)en.setDate(en.getDate()+1);Object.assign(s,{start:st.toISOString(),end:en.toISOString(),travel:Number(fd.travel)||0,production:fd.production,notes:fd.notes});saveData();render();toast('Shift updated')
  });
  $('#deleteShiftBtn').onclick=()=>{
    if(confirm('Delete this shift permanently?')){
      data.shifts=data.shifts.filter(x=>x.id!==s.id);saveData();$('#modal').close('deleted');render();toast('Shift deleted')
    }
  };
}
